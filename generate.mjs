...
function buildCells(weeks, theme, COLS) {
  ...
  weeks.forEach((week, col) => {
    week.contributionDays.forEach((day, row) => {
      // Always use our pink theme palette — never use day.color (GitHub's green)
      const level =
        typeof day.level === "number"
          ? day.level
          : countToLevel(day.contributionCount);
      const color = theme.levelColors[level] || theme.levelColors[0];
      cells.push({
        col,
        row,
        x: GRID_X + col * STEP,
